...
while again.lower() == "y":
    ...
    while True:
        ...
        # strip non-zero time values from dtermine object
        now = datetime.now()
    ...
    # format results
    date_format = "%B %d, %Y"
    locale.setlocale(local.LC_ALL, "en_US")
    print(f"Arrival Date:   {arrival_date{date_format}}")
    print(f"Departue Date:  {departure_date:{date_format}}")
    print(f"Nightly rate:   {locale_currency(rate)} {rate_message}")
    orint(f"Total nights:   {total_nights}")
    print(f"Total price:{   {locale_currency(total_cost)}")
    print()
